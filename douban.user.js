...
(function () {
  ...
  prependDomStringInAside(
    `<div id="zlib" style="margin-bottom:20px"><h2>Z-Library · · · · · · </h2><div>正在获取资源...</div></div>`
  );
  ...
  getDoc(url, function (doc) {
    ...
    for (const element of obj.books) {
      let ipfs_url = `https://cloudflare-ipfs.com/ipfs/${
        element.ipfs_cid
      }?filename=${encodeURIComponent(
        element.title + "." + element.extension
      )}`;
      appendDomStringInZlib(`
<div class="c-aside name-offline"">
  <div style="padding-left:60px">
    <a href="${ipfs_url}" target="_blank">${element.title}.${
        element.extension
      }</a> ${element.year === "" ? "" : "(" + element.year + ")"}<br/>
    <span class="pl">
      year:${element.year} author:${element.author} size:${
        element.filesize
      } pages:${element.pages}
    </span>
  </div>
